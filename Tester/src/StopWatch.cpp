...
StopWatch::StopWatch(const std::string& name, bool start, bool printResult)
	: m_Name(name), m_PrintResult(printResult), m_StartTime(0), m_EndTime(0)
{
	if (start)
		Start();
}
...
void StopWatch::Stop()
{
	...
	if (m_PrintResult)
		std::cout << "Stop watch '" << m_Name << "' ended with elapsed time " << GetElapsedTimeMilliSeconds() << "ms." << std::endl;
	m_Started = false;
}